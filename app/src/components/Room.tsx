...
export const Room = ({
  ...
  const { data } = useDocument({ path: `rooms/${id}` })
  ...
  const joinTheRoom = () => {
    ...
    useDocumentUtils({ path: `users/${userUid}` }).updateDocument({
      rooms: justData,
    })
  }
  const notJoinTheRoom = () => {
    let justData = roomData.map((item) => {
      if (item.id == id) item.status = 'declined'
            ...
            {users?.map((item, index) => {
              return (
                <UserAvatar
                  ...
                  name="Avishay Bar"
                  key={index}
                  bgColor={colors[Math.floor(Math.random() * 6)]}
                />
              )
            })}
          </Queue>
          <Text style={styles.roomName}> - {myFee}</Text>
        </Queue>

        {isPending && (
          <Stack size={5} alignItems="center">
            <Text style={{ textAlign: 'center', maxWidth: '80%' }}>
              Та {roomName} нэртэй өрөөнд орохдоо итгэлтэй байна уу
            </Text>
            <Queue justifyContent="space-between">
